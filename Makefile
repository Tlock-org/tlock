...
HTTPS_GIT := github.com/rollchains/tlock.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=tlock \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=tlockd \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/tlockd ./cmd/tlockd
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/tlockd.exe ./cmd/tlockd
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/tlockd
...
draw-deps:
	...
	@goviz -i ./cmd/tlockd -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c tlock --local -f chains.yaml
endif
