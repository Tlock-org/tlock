...
# then log output of file /code/bin/tlockd
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/tlockd \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/tlockd | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/tlockd /usr/bin/tlockd
