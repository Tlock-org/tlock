...
var maccPerms = map[string][]string{
	...
	//posttypes.ModuleName: {authtypes.Minter, authtypes.Burner},
}
...
func NewChainApp(
	...
	app.PostKeeper = postkeeper.NewKeeper(
		appCodec,
		keys[posttypes.StoreKey], // storeKey
		runtime.NewKVStoreService(keys[posttypes.StoreKey]),
		logger,
		authtypes.NewModuleAddress(govtypes.ModuleName).String(),
		app.AccountKeeper,
		app.BankKeeper,
	...
	maccPerms[posttypes.ModuleName] = []string{authtypes.Minter, authtypes.Burner}
	...
	// see cmd/tlockd/root.go: 206 - 214 approx
...
func (app *ChainApp) InitChainer(ctx sdk.Context, req *abci.RequestInitChain) (*abci.ResponseInitChain, error) {
	...
	// get and set post module account
	postModuleAccount := app.AccountKeeper.GetModuleAccount(ctx, posttypes.ModuleName)
	if postModuleAccount == nil {
		// if the module account is not exist, to create
		postModuleAccount = authtypes.NewEmptyModuleAccount(posttypes.ModuleName, authtypes.Minter, authtypes.Burner)
		app.AccountKeeper.SetModuleAccount(ctx, postModuleAccount)
...
func BlockedAddresses() map[string]bool {
	...
	delete(modAccAddrs, authtypes.NewModuleAddress(posttypes.ModuleName).String())
